...
async function fetchActivities() {
  ...
  try {
    ...
    const response = await fetch(
      `${base_url}api/child/lesson/${lesson.value.lesson_id}/activities`,
      {
...
async function handleFileUpload(event: Event) {
  ...
  try {
    ...
    const response = await fetch(
      `${base_url}api/child/activity/${selectedActivityIdForUpload.value}/submit`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
...
async function viewSubmission(historyItem: ActivityHistoryItem) {
  try {
    ...
    const response = await fetch(
      `${base_url}api/child/activity/history/${historyItem.activity_history_id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
